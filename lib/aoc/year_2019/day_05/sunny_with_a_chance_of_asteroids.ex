defmodule Aoc.Year2019.Day05.SunnywithaChanceofAsteroids do
  ...
  *what diagnostic code does the program produce?*
  ...
  def step(program, inputs, outputs \\ [], pc \\ 0) do
    instruction = Arrays.get(program, pc)
    {opcode, mode1, mode2} = decode(instruction)
  ...
  defp read_operands(program, pc, num) do
    1..num |> Enum.map(&Arrays.get(program, pc + &1)) |> List.to_tuple()
  end

  defp read(program, address, 0), do: Arrays.get(program, address)
  defp read(_program, address, 1), do: address
