defmodule Aoc.Year2019.Day02.ProgramAlarm do
  ...
  *where* the inputs and outputs are, not their values. 
  ...
  `1,9,10,3,2,3,11,0,99,30,40,50`For the purposes of illustration, here is the same program split into multiple
  lines: 
  ...
  *parameters*. For example, in the instruction `1,2,3,4`, `1` is the opcode; `2`,
  ...
  def run_program(current, before \\ [], pc \\ 0)
  def run_program([99 | rest], before, _pc) do
    before ++ [99] ++ rest
  end 

  def run_program(current = [opcode, opa, opb, opc | _], before, pc) do
    result = case opcode do
      1 -> 
        read_at(opa, current, before, pc) + read_at(opb, current, before, pc)
      2 -> 
        read_at(opa, current, before, pc) * read_at(opb, current, before, pc)
    end

    {current, before} = cond do
      opc < pc ->
        {current, set(before, opc, result)}
      opc >= pc ->
        {set(current, opc - pc, result), before}
    end

    {append, current} = Enum.split(current, 4)
    run_program(current, before ++ append, pc + 4)
  end 

  defp read_at(position, _tail, head, pc) when position < pc do
    Enum.at(head, position)
  end

  defp read_at(position, tail, _head, pc) do
    Enum.at(tail, position - pc)
  end

  def set(list, position, value, rest \\ [])
  def set([_head | tail], 0, value, rest) do
    rest ++ [value] ++ tail
  end
  def set([head | tail], position, value, rest) do
    set(tail, position - 1, value, rest ++ [head])
  end
