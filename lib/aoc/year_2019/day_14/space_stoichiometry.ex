defmodule Aoc.Year2019.Day14.SpaceStoichiometry do
  @moduledoc """
  ...
  into `D`, `D` into `E`, and finally `E` into `FUEL`. (30 `A` is produced because
  its reaction requires that it is created in increments of 10.)
  ...
  """
  def part_1(input, compound \\ "FUEL") do
    conversions = input |> parse

    {used, _leftovers} = run(conversions, 1, compound)
    used
  end

  def run(conversions, quantity, compound, used \\ 0, leftovers \\ %{})
  def run(_conversions, 0, _compound, used, leftovers), do: {used, leftovers}

  def run(conversions, quantity, compound, used, leftovers) do
    {increment, parts} = Map.get(conversions, compound)
    batches = Float.ceil(quantity / increment) |> trunc

    {used, leftovers} =
      case parts do
        [{req, "ORE"}] ->
          {batches * req + used, leftovers}

        parts ->
          Enum.reduce(parts, {used, leftovers}, fn part, {used, leftovers} ->
            {part_quantity, part_compound} = part
            required_quantity = part_quantity * batches

            inventory = Map.get(leftovers, part_compound, 0)
            taken = Enum.min([inventory, required_quantity])
            extra_quantity = required_quantity - taken

            {used, leftovers} = run(conversions, extra_quantity, part_compound, used, leftovers)

            # IO.inspect("Taking #{required_quantity} of #{part_compound}")

            leftovers =
              Map.update(leftovers, part_compound, -required_quantity, fn cur ->
                cur - required_quantity
              end)
    ...
    leftovers =
      Map.update(leftovers, compound, increment * batches, fn cur -> cur + increment * batches end)

    # IO.inspect("Adding #{increment * batches} of #{compound}")
  ...
  """
  def part_2(input, compound \\ "FUEL") do
    conversions = input |> parse
    {used, _leftovers} = run(conversions, 1, compound)
    lower_bound = div(1_000_000_000_000, used)
    run_part2(conversions, compound, lower_bound + 1, 100_000)
  end

  def run_part2(conversions, compound, guess, step \\ 1000) do
    {used, _} = run(conversions, guess, compound)

    cond do
      used > 1_000_000_000_000 && step == 1 ->
        guess - 1

      used > 1_000_000_000_000 && step > 1 ->
        run_part2(conversions, compound, guess - step, div(step, 10))

      used == 1_000_000_000_000 ->
        guess

      used < 1_000_000_000_000 ->
        run_part2(conversions, compound, guess + step, step)
    end
  end

  def parse(input) do
    input |> String.split("\n") |> Enum.map(&parse_row/1) |> Map.new()
  end

  def parse_row(row) do
    parts = row |> String.split([", ", " => "]) |> Enum.map(&parse_part/1)

    {parts, last} = Enum.split(parts, length(parts) - 1)

    [{quantity, compound}] = last

    {compound, {quantity, parts}}
  end

  defp parse_part(part) do
    [quantity, compound] = String.split(part, " ")
    {String.to_integer(quantity), compound}
  end
end
