defmodule Aoc.Year2019.Day12.TheNBodyProblem do
  ...
  *velocity*. Time progresses by one step once all of the positions are updated.
  ...
  a `x` position of `5`, then Ganymede's `x` velocity *changes by +1* (because `5
  > 3`) and Callisto's `x` velocity *changes by -1* (because `3 < 5`). However, if
  the positions on a given axis are the same, the velocity on that axis *does not
  ...
  Sum of total energy: 36 + 45 + 80 + 18 = *179*`In the above example, adding together the total energy for all moons after 10
  ...
  Sum of total energy: 290 + 608 + 574 + 468 = *1940*`*What is the total energy in the system* after simulating the moons given in
  ...
  def part_1(input, steps \\ 1000) do
    input |> parse |> simulate(steps)
  end

  def simulate(moons, 0) do
    moons |> transpose |> Enum.map(&energy/1) |> Enum.sum()
  end

  def simulate(moons, steps) do
    moons |> Enum.map(&simulate_axis/1) |> simulate(steps - 1)
  end

  def simulate_axis(axis) do
    axis |> Enum.map(&step_velo(&1, axis)) |> Enum.map(&apply_velo/1)
  end

  def step_velo(moon, moons) do
    Enum.reduce(moons, moon, fn other_moon, moon ->
      apply_gravity(moon, other_moon)
    end)
  end

  def apply_gravity({z, dz}, {w, _dw}), do: {z, dz + gravity_change(z, w)}
  def apply_velo({z, dz}), do: {z + dz, dz}

  def energy([{x, dx}, {y, dy}, {z, dz}]) do
    potential = abs(x) + abs(y) + abs(z)
    kinetic = abs(dx) + abs(dy) + abs(dz)
    potential * kinetic
  end

  def gravity_change(pos, pos), do: 0
  def gravity_change(pos, other_pos) when pos < other_pos, do: 1
  def gravity_change(pos, other_pos) when pos > other_pos, do: -1
  ...
  """
  def part_2(input) do
    [xs, ys, zs] = input |> parse
    [rx, ry, rz] = [xs, ys, zs] |> Enum.map(&simulate_part_2(&1, &1))
    lcm(rx, ry) |> lcm(rz)
  end

  def simulate_part_2(ws, original, steps \\ 0)

  def simulate_part_2(ws, ws, steps) when steps != 0, do: steps

  def simulate_part_2(ws, original, steps) do
    simulate_part_2(simulate_axis(ws), original, steps + 1)
  end

  def parse(input) do
    input |> String.split("\n") |> Enum.map(&parse_row/1) |> transpose
  end

  def parse_row(row) do
    split = row |> String.split(["<", ",", "=", ">"])
    [x, y, z] = [2, 4, 6] |> Enum.map(&Enum.at(split, &1)) |> Enum.map(&String.to_integer/1)
    [{x, 0}, {y, 0}, {z, 0}]
  end
