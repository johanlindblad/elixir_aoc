defmodule Aoc.Year2019.Day01.TheTyrannyoftheRocketEquation do
  ...
  *What is the sum of the fuel requirements* for all of the modules on your
  ...
  def part_1(input) do
    input
    |> parse_input
    |> Enum.map(&module_fuel_required/1)
    |> Enum.sum()
  end

  def module_fuel_required(mass) when is_integer(mass) do
    div(mass, 3) - 2
  end
  ...
  def part_2(input) do
    input
    |> parse_input
    |> Enum.map(&module_fuel_required_inclusive/1)
    |> Enum.sum()
  end

  def module_fuel_required_inclusive(mass) when is_integer(mass) do
    case module_fuel_required(mass) do
      required when required < 0 -> 0
      required -> required + module_fuel_required_inclusive(required)
    end
  end
